...
class Kohana_Database_MySQLi extends Database {
	...
	public function connect()
	{
		...
		extract($this->_config['connection'] + array(
			...
			'socket'     => ini_get('mysqli_default_socket'),
			'persistent' => FALSE, // only php5.3 >=
		));
		if ($persistent)
		{
			$hostname = 'p:' . $hostname;
		...
		if ( ! empty($this->_config['connection']['variables']))
		{
			...
			$this->_connection->query('SET '.implode(', ', $variables), $this->_connection);
		}
	}
	...
	public function query($type, $sql, $as_object = FALSE, array $params = NULL)
	{
		...
		// Execute the query
		// TODO: Allow async query for php 5.3 >=
	 ...
	 *
	 * @todo php5.5 begin_transaction() support
